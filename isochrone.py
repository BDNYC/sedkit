...
# Fails RTD build for some reason
    ...
    EVO_MODELS = ['COND03', 'dmestar_solar', 'DUSTY00', 'f2_solar_age', 'hybrid_solar_age', 'nc+03_age', 'nc-03_age', 'nc_solar_age', 'parsec12_solar','ATMO_NEQ_strong','ATMO_NEQ_strong_MIRI']
...
class Isochrone:
    ...
    def evaluate(self, xval, age, xparam, yparam, plot=False):
        ...
        # s_a = np.random.uniform(mu_a - sigma_a, mu_a + sigma_a, 1000)
        ...
        # # error = std/np.sqrt(len(values_list))
        ...
        if plot:
            ...
            show(fig)
        # changed nominal to average if using other result mode
        # # Balk at nans
        if np.isnan(nominal):
