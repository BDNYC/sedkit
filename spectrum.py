...
class Spectrum:
    ...
    def integrate(self, units=None, n_samples=10):
        ...
        # delta_wl = spec[0][1:] - spec[0][:-1]
        # delta_wl = np.insert(delta_wl,delta_wl.size, delta_wl[-1])
        # val = (np.sum(spec[1] * delta_wl)* m).to(units)
        ...
        if self.unc is None:
            ...
            # out = open('SEDkit.dat','w')
            # for i in range(len(spec[0])):
            #     out.write('%11.3F %11.5E %11.5E \n' %(spec[0][i],spec[1][i],spec[2][i]))
            # out.close()

            # print("\n\n\n THESE ARE THE MINS", np.min(spec[1]), np.min(spec[2]),np.min(usamp))
            # print("\n\n\n THESE ARE THE MAXS", np.max(spec[1]), np.max(spec[2]),np.max(usamp))
            #
            # print("\nThese are usamp errors",usamp[:11],len(usamp))
            # print("\n\n\nTHIS IS THE ERROR",err)
            # print(uvals,np.std(uvals))
